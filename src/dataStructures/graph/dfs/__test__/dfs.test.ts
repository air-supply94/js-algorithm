...
describe('depthFirstSearch', () => {
  test('should perform DFS operation on graph', () => {
    ...
    expect(enterVertexCallback)
      .toHaveBeenCalledTimes(graph.getAllVertices().length);
    expect(leaveVertexCallback)
      .toHaveBeenCalledTimes(graph.getAllVertices().length);
    ...
    for (let callIndex = 0; callIndex < graph.getAllVertices().length; callIndex += 1) {
      ...
      expect(params.currentVertex)
        .toEqual(enterVertexParamsMap[callIndex].currentVertex);
      expect(params.previousVertex)
        .toEqual(enterVertexParamsMap[callIndex].previousVertex);
    }
    ...
    for (let callIndex = 0; callIndex < graph.getAllVertices().length; callIndex += 1) {
      ...
      expect(params.currentVertex)
        .toEqual(leaveVertexParamsMap[callIndex].currentVertex);
      expect(params.previousVertex)
        .toEqual(leaveVertexParamsMap[callIndex].previousVertex);
    }
  });

  test('allow users to redefine vertex visiting logic', () => {
    const graph = new Graph(true);
