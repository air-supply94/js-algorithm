...
test('skipList for number', () => {
  ...
  data.forEach((item, index) => {
    expect(skipList.insert(item))
      .toBeTruthy();
    expect(skipList.insert(item))
      .toBeFalsy();
    expect(skipList.search(item))
      .toBe(item);
    expect(printSkipList(skipList))
      .toBe(data.slice(0, index + 1)
        .join(','));
  });
  ...
  data.forEach((item, index) => {
    expect(skipList.remove(item))
      .toBeTruthy();
    expect(skipList.search(item))
      .toBeNull();
    expect(printSkipList(skipList))
      .toBe(data.slice(index + 1)
        .join(','));
  });
});
...
test('skipList for object', () => {
  ...
  expect(skipList.remove({ value: -1 }))
    .toBeFalsy();
  ...
  expect(skipList.remove({ value: 100 }))
    .toBeFalsy();
