...
describe('BinarySearchTree', () => {
  test('should create binary search tree', () => {
    ...
    bst.insert(2);
    bst.insert(0);
    expect(findReplaceNode(bst.root, 1, bst.comparator).parent.value).toBe(2);
  });
  ...
  test('should be traversed to sorted array', () => {
    ...
    bst.insert(25);
    bst.insert(6);
    expect(bst.findMin().value).toBe(-20);
    expect(bst.findMax().value).toBe(25);

    expect(bst.toString()).toBe('-20,-10,6,10,20,25');
    expect(getHeight(bst.root)).toBe(2);

    bst.insert(4);

    expect(bst.toString()).toBe('-20,-10,4,6,10,20,25');
    expect(getHeight(bst.root)).toBe(3);
  });
  ...
  test('should insert in itself if it is empty', () => {
    ...
    expect(bstNode.root.value).toBe(1);
    expect(bstNode.root.left).toBeNull();
    expect(bstNode.root.right).toBeNull();
  });
  ...
  test('should remove nodes with two children', () => {
    ...
    expect(removeNode.value).toBe(20);
    expect(removeNode.parent.value).toBe(30);

    expect(bstRootNode.root.right.parent.value).toBe(10);
    expect(bstRootNode.toString()).toBe('5,10,15,25,30');

    expect(bstRootNode.remove(15).parent.value).toBe(25);

    expect(bstRootNode.toString()).toBe('5,10,25,30');

    const removeNode2 = bstRootNode.remove(10);
    expect(removeNode2.parent.value).toBe(30);
  ...
  test('should traverse node', () => {
    ...
    expect(bst.traversePreOrder().join()).toBe('');
    expect(bst.traverseInOrder().join()).toBe('');
    expect(bst.traverseAfterOrder().join()).toBe('');
    expect(bst.traverseLevelOrder().join()).toBe('');

    bst.insert(4);
    bst.insert(6);
    expect(bst.traverseAfterOrder().join()).toBe('6,4');
    ...
    expect(result.toString()).toBe('4,2,1');
    result = [];
    ...
    expect(result.toString()).toBe('1,3,2');
    result = [];

    bst.traverseLevelOrderCallback(callback);
    expect(result.toString()).toBe('4,2,6');
    result = [];

    expect(bst.traversePreOrder().join()).toBe('4,2,1,3,6,5,7');

    expect(bst.traverseInOrder().join()).toBe('1,2,3,4,5,6,7');

    bst.insert(0);
    expect(bst.traverseAfterOrder().join()).toBe('0,1,3,2,5,7,6,4');
    bst.remove(0);

    expect(bst.traverseLevelOrder().join()).toBe('4,2,6,1,3,5,7');
  });
});
