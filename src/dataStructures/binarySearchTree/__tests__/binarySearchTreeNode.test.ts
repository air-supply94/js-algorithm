...
describe('BinarySearchTreeNode', () => {
  ...
  test('should set parent', () => {
    ...
    const rightNode = new BinarySearchTreeNode(3);
    const rootNode = new BinarySearchTreeNode(2);
    expect(removeChild(rightNode, leftNode)).toBeFalsy();
  ...
  test('should remove child node', () => {
    ...
    expect(nodeToString(rootNode)).toBe('1,2,3');

    expect(removeChild(rootNode, rootNode.left)).toBe(true);
    expect(nodeToString(rootNode)).toBe('2,3');

    expect(removeChild(rootNode, rootNode.right)).toBe(true);
    expect(nodeToString(rootNode)).toEqual('2');

    expect(removeChild(rootNode, rootNode.right)).toBe(false);
    expect(nodeToString(rootNode)).toBe('2');
  });
  ...
  test('should replace child node', () => {
    ...
    expect(nodeToString(rootNode)).toEqual('1,2,3,5');

    expect(replaceChild(rootNode, rootNode.right, rootNode.right.right)).toBe(true);
    expect(rootNode.right.value).toBe(5);
    expect(rootNode.right.right).toBeNull();
    expect(nodeToString(rootNode)).toBe('1,2,5');

    expect(replaceChild(rootNode, rootNode.right, rootNode.right.right)).toBe(false);
    expect(nodeToString(rootNode)).toBe('1,2,5');

    expect(replaceChild(rootNode, rootNode.right, replacementNode)).toBe(true);
    expect(nodeToString(rootNode)).toBe('1,2,5');

    expect(replaceChild(rootNode, rootNode.left, replacementNode)).toBe(true);
    expect(nodeToString(rootNode)).toBe('5,2,5');

    expect(replaceChild(rootNode, new BinarySearchTreeNode(), new BinarySearchTreeNode())).toBe(false);
  });
  ...
  test('should calculate node height', () => {
    ...
    expect(getHeight(root)).toBe(0);
    expect(getBalanceFactor(root)).toBe(0);

    setLeft(root, left);
    setRight(root, right);

    expect(getHeight(root)).toBe(1);
    expect(getHeight(left)).toBe(0);
    expect(getBalanceFactor(root)).toBe(0);
    ...
    expect(getHeight(left)).toBe(1);
    expect(getHeight(grandLeft)).toBe(0);
    expect(getHeight(grandRight)).toBe(0);
    expect(getBalanceFactor(root)).toBe(1);

    setLeft(grandLeft, grandGrandLeft);

    expect(getHeight(root)).toBe(3);
    expect(getHeight(left)).toBe(2);
    expect(getHeight(grandLeft)).toBe(1);
    expect(getHeight(grandRight)).toBe(0);
    expect(getHeight(grandGrandLeft)).toBe(0);
    expect(getBalanceFactor(root)).toBe(2);
  });
  ...
  test('should be possible to create node with object as a value', () => {
    ...
    expect(nodeToString(node1)).toBe('object_1');
    expect(nodeToString(node2)).toBe('[object Object]');
  });
  ...
  test('should detect right uncle', () => {
    ...
    expect(getUncle(null)).toBeNull();
    expect(getUncle(grandParent)).toBeNull();
    expect(getUncle(parent)).toBeNull();

    setLeft(grandParent, parent);

    expect(getUncle(parent)).toBeNull();
    expect(getUncle(child)).toBeNull();
    ...
    expect(getUncle(parent)).toBeNull();
    expect(getUncle(child)).toBeDefined();
    expect(getUncle(child)).toEqual(uncle);
  });
  ...
  test('should be possible to copy node', () => {
    ...
    expect(nodeToString(root)).toBe('left,root,right');
    ...
    expect(nodeToString(newRoot)).toBe('new_left,new_root,new_right');
