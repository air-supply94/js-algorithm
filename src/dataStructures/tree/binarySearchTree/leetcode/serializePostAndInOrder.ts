...
function recursion<T = unknown>(postorder: T[], postStartIndex: number, postEndIndex: number, inorder: T[], inStartIndex: number, inEndIndex: number) {
  ...
  root.setLeft(recursion(postorder, postStartIndex, postStartIndex + size - 1, inorder, inStartIndex, equalIndex - 1));
  root.setRight(recursion(postorder, postStartIndex + size, postEndIndex - 1, inorder, equalIndex + 1, inEndIndex));
  return root;
}
